...
- Then call the `codex_ask` tool exactly once with two options, putting the recommended option first and suffixing its label with `(Recommended)`:
  - `Wait for results`
  - `Run in background`
...
- Run:
```bash
node "${PI_CODEX_ROOT}/scripts/codex-companion.mjs" adversarial-review "$ARGUMENTS"
```
...
```bash
nohup node "${PI_CODEX_ROOT}/scripts/codex-companion.mjs" adversarial-review "$ARGUMENTS" >/dev/null 2>&1 &
```
- Tell the user: "Codex adversarial review started in the background. Check `/codex:status` for progress."
